...
/**
 * <p>A control that allows switching between a group of {@link FX8Tab Tabs}.  Only one tab
...
public class FX8TabPane extends Control {
    ...
    public FX8TabPane() {
        ...
        tabs.addListener(new ListChangeListener<FX8Tab>() {
            @Override public void onChanged(Change<? extends FX8Tab> c) {
                while (c.next()) {
                    for (FX8Tab tab : c.getRemoved()) {
                        if (tab != null && !getTabs().contains(tab)) {
                            tab.setTabPane(null);
                        }
                    }

                    for (FX8Tab tab : c.getAddedSubList()) {
                        if (tab != null) {
                            tab.setTabPane(FX8TabPane.this);
                        }
                    }
                }
            }
     ...
     */
    public final ObservableList<FX8Tab> getTabs() {
        return tabs;
    }

    private ObjectProperty<SingleSelectionModel<FX8Tab>> selectionModel = new SimpleObjectProperty<SingleSelectionModel<FX8Tab>>(this, "selectionModel");
     ...
     */
    public final void setSelectionModel(SingleSelectionModel<FX8Tab> value) { selectionModel.set(value); }

    /**
     * <p>Gets the model used for tab selection.</p>
     */
    public final SingleSelectionModel<FX8Tab> getSelectionModel() { return selectionModel.get(); }

    /**
     * The selection model used for selecting tabs.
     */
    public final ObjectProperty<SingleSelectionModel<FX8Tab>> selectionModelProperty() { return selectionModel; }
    ...
    /** {@inheritDoc} */
    /* FIXME @Override */protected Skin<?> createDefaultSkin() {
        return new FX8TabPaneSkin(this);
    }
    ...
    private static class StyleableProperties {
        private static final StyleableProperty<FX8TabPane,Number> TAB_MIN_WIDTH =
                new StyleableProperty<FX8TabPane,Number>("-fx-tab-min-width",
                SizeConverter.getInstance(), DEFAULT_TAB_MIN_WIDTH) {

            @Override
            public boolean isSettable(FX8TabPane n) {
                return n.tabMinWidth == null || !n.tabMinWidth.isBound();
            }

            @Override
            public WritableValue<Number> getWritableValue(FX8TabPane n) {
                return n.tabMinWidthProperty();
            }
        };

        private static final StyleableProperty<FX8TabPane,Number> TAB_MAX_WIDTH =
                new StyleableProperty<FX8TabPane,Number>("-fx-tab-max-width",
        ...
        private static final StyleableProperty<FX8TabPane,Number> TAB_MIN_HEIGHT =
                new StyleableProperty<FX8TabPane,Number>("-fx-tab-min-height",
        ...
        private static final StyleableProperty<FX8TabPane,Number> TAB_MAX_HEIGHT =
                new StyleableProperty<FX8TabPane,Number>("-fx-tab-max-height",
    ...
    public static List<StyleableProperty> impl_CSS_STYLEABLES() {
        return FX8TabPane.StyleableProperties.STYLEABLES;
    }
    ...
    static class TabPaneSelectionModel extends SingleSelectionModel<FX8Tab> {
        private final FX8TabPane tabPane;

        public TabPaneSelectionModel(final FX8TabPane t) {
            if (t == null) {
            ...
            final ListChangeListener<FX8Tab> itemsContentObserver = new ListChangeListener<FX8Tab>() {
                @Override public void onChanged(Change<? extends FX8Tab> c) {
                    while (c.next()) {
                        for (FX8Tab tab : c.getRemoved()) {
        ...
        @Override public void select(FX8Tab tab) {
            final int itemCount = getItemCount();

            for (int i = 0; i < itemCount; i++) {
                final FX8Tab value = getModelItem(i);
        ...
        @Override protected FX8Tab getModelItem(int index) {
            final ObservableList<FX8Tab> items = tabPane.getTabs();
