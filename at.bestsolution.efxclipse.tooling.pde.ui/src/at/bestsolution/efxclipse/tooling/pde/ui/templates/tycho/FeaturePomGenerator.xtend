...
class FeaturePomGenerator implements Generator<DynamicFile> {
	override generate(DynamicFile file, Map<String,Object> data) {
		...
		val pomData = new PomData(productName + " - application feature", toPomGroupId(bundleId), bundleId +".feature", toPomGroupId(bundleId), bundleId+".releng", toPomVersion(bundleVersion), "../"+projectName+".releng")
