...
/**
 * <p>A control that allows switching between a group of {@link FX2Tab Tabs}.  Only one tab
...
public class FX2TabPane extends FXTabPane<FX2Tab> {
    ...
    public FX2TabPane() {
        ...
        tabs.addListener(new ListChangeListener<FX2Tab>() {
            @Override public void onChanged(Change<? extends FX2Tab> c) {
                while (c.next()) {
                    for (FX2Tab tab : c.getRemoved()) {
                        if (tab != null && !getTabs().contains(tab)) {
                            tab.setTabPane(null);
                        }
                    }

                    for (FX2Tab tab : c.getAddedSubList()) {
                        if (tab != null) {
                            tab.setTabPane(FX2TabPane.this);
                        }
                    }
                }
            }
     ...
     */
    public final ObservableList<FX2Tab> getTabs() {
        return tabs;
    }

    private ObjectProperty<SingleSelectionModel<FX2Tab>> selectionModel = new SimpleObjectProperty<SingleSelectionModel<FX2Tab>>(this, "selectionModel");
     ...
     */
    public final void setSelectionModel(SingleSelectionModel<FX2Tab> value) { selectionModel.set(value); }

    /**
     * <p>Gets the model used for tab selection.</p>
     */
    public final SingleSelectionModel<FX2Tab> getSelectionModel() { return selectionModel.get(); }

    /**
     * The selection model used for selecting tabs.
     */
    public final ObjectProperty<SingleSelectionModel<FX2Tab>> selectionModelProperty() { return selectionModel; }
    ...
    private static class StyleableProperties {
        private static final StyleableProperty<FX2TabPane,Number> TAB_MIN_WIDTH =
                new StyleableProperty<FX2TabPane,Number>("-fx-tab-min-width",
                SizeConverter.getInstance(), DEFAULT_TAB_MIN_WIDTH) {

            @Override
            public boolean isSettable(FX2TabPane n) {
                return n.tabMinWidth == null || !n.tabMinWidth.isBound();
            }

            @Override
            public WritableValue<Number> getWritableValue(FX2TabPane n) {
                return n.tabMinWidthProperty();
            }
        };

        private static final StyleableProperty<FX2TabPane,Number> TAB_MAX_WIDTH =
                new StyleableProperty<FX2TabPane,Number>("-fx-tab-max-width",
        ...
        private static final StyleableProperty<FX2TabPane,Number> TAB_MIN_HEIGHT =
                new StyleableProperty<FX2TabPane,Number>("-fx-tab-min-height",
        ...
        private static final StyleableProperty<FX2TabPane,Number> TAB_MAX_HEIGHT =
                new StyleableProperty<FX2TabPane,Number>("-fx-tab-max-height",
    ...
    public static List<StyleableProperty> impl_CSS_STYLEABLES() {
        return FX2TabPane.StyleableProperties.STYLEABLES;
    }
    ...
    static class TabPaneSelectionModel extends SingleSelectionModel<FX2Tab> {
        private final FX2TabPane tabPane;

        public TabPaneSelectionModel(final FX2TabPane t) {
            if (t == null) {
            ...
            final ListChangeListener<FX2Tab> itemsContentObserver = new ListChangeListener<FX2Tab>() {
                @Override public void onChanged(Change<? extends FX2Tab> c) {
                    while (c.next()) {
                        for (FX2Tab tab : c.getRemoved()) {
        ...
        @Override public void select(FX2Tab tab) {
            final int itemCount = getItemCount();

            for (int i = 0; i < itemCount; i++) {
                final FX2Tab value = getModelItem(i);
        ...
        @Override protected FX2Tab getModelItem(int index) {
            final ObservableList<FX2Tab> items = tabPane.getTabs();
