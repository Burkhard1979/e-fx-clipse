...
public abstract class AbstractMarkerAnnotationModel extends AnnotationModel implements IPersistableAnnotationModel {
	...
	protected void connected() {
		...
		try {
			...
			if (x.getStatus().getCode() != IResourceStatus.RESOURCE_NOT_FOUND)
				handleCoreException(x, EditorMessages.AbstractMarkerAnnotationModel_connected);
		}
	...
	private void installMarkerUpdaters() {
		...
		// populate list
		IExtensionPoint extensionPoint= Platform.getExtensionRegistry().getExtensionPoint(TextEditorPlugin.PLUGIN_ID, "markerUpdaters"); //$NON-NLS-1$
			...
			if (markerAnnotations.size() > 0) {
				...
				if (modelInitiated) {
					...
					try {
						...
						handleCoreException(x, EditorMessages.AbstractMarkerAnnotationModel_removeAnnotations);
					}
					listenToMarkerChanges(true);
	...
	private IMarkerUpdater createMarkerUpdater(IConfigurationElement element) {
		try {
			...
			handleCoreException(x, EditorMessages.AbstractMarkerAnnotationModel_createMarkerUpdater);
		}
